 ...
 * edge.
 */
typedef void EdgeStyleFunction(CellState state, CellState source, CellState target, List<Point2d> points, List<Point2d> result);
...
class EdgeStyle {
  ...
  static EdgeStyleFunction EntityRelation = (CellState state, CellState source, CellState target, List<Point2d> points, List<Point2d> result) {
    ...
    if (source != null && target != null) {
      ...
      Point2d dep = new Point2d(x0 + dx, y0);
      result.add(dep);

      dx = (isTargetLeft) ? -seg : seg;
      Point2d arr = new Point2d(xe + dx, ye);

      // Adds intermediate points if both go out on same side
      if (isSourceLeft == isTargetLeft) {
        double x = (isSourceLeft) ? Math.min(x0, xe) - segment : Math.max(x0, xe) + segment;
        result.add(new Point2d(x, y0));
        result.add(new Point2d(x, ye));
      } else if ((dep.getX() < arr.getX()) == isSourceLeft) {
        double midY = y0 + (ye - y0) / 2;
        result.add(new Point2d(dep.getX(), midY));
        result.add(new Point2d(arr.getX(), midY));
      }
    ...
    if (source != null && target != null) {
      ...
      if (result.length == 1) {
        if (pt != null) {
          ...
          result.add(new Point2d(x, t + (b - t) / 2));
        }
      }
    }
  };
    ...
    if (source != null && target != null) {
      ...
      if (result.length == 1) {
        if (pt != null) {
          ...
          result.add(new Point2d(l + (r - l) / 2, y));
        }
      }
    }
  };
  ...
  static EdgeStyleFunction SegmentConnector = (CellState state, CellState source, CellState target, List<Point2d> hints, List<Point2d> result) {
    ...
    if (hints != null && hints.length > 0) {
      ...
      for (int i = 0; i < 2; i++) {
        ...
        bool inHozChan = currentTerm != null && (currentHint.getY() >= currentTerm.getY() && currentHint.getY() <= currentTerm.getY() + currentTerm.getHeight());
        bool inVertChan = currentTerm != null && (currentHint.getX() >= currentTerm.getX() && currentHint.getX() <= currentTerm.getX() + currentTerm.getWidth());
  ...
  static int SIDE_MASK = LEFT_MASK | TOP_MASK | RIGHT_MASK | BOTTOM_MASK;
  ...
  static EdgeStyleFunction OrthConnector = (CellState state, CellState source, CellState target, List<Point2d> points, List<Point2d> result) {
    ...
    if (source != null && target != null) {
      ...
      for (int i = 0; i <= currentIndex; i++) {
        result.add(new Point2d(wayPoints1[i][0], wayPoints1[i][1]));
      }
