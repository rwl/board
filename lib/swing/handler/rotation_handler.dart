...
class RotationHandler extends MouseAdapter {
  ...
  void start(MouseEvent e) {
    ...
    _first = SwingUtilities.convertPoint(e.getComponent(), e.getPoint(), _graphComponent.getGraphControl());
  ...
  void mouseMoved(MouseEvent e) {
    if (_graphComponent.isEnabled() && isEnabled()) {
      if (_handle.getParent() != null && e.getSource() == _handle /* mouse hits handle */) {
        ...
        e.consume();
      } else if (_currentState == null || !_currentState.getRectangle().contains(e.getPoint())) {
        CellState eventState = _graphComponent.getGraph().getView().getState(_graphComponent.getCellAt(e.getX(), e.getY(), false));
        ...
        if (_currentState != state) {
          ...
          if (_currentState == null && _handle.getParent() != null) {
            ...
            _handle.setLocation((int)(_currentState.getX() + _currentState.getWidth() - _handle.getWidth() - 4), (int)(_currentState.getY() + _currentState.getHeight() - _handle.getWidth() - 4));
          }
        }
      }
    }
  ...
  void mouseDragged(MouseEvent e) {
    if (_graphComponent.isEnabled() && isEnabled() && !e.isConsumed() && _first != null) {
      ...
      awt.Point pt = SwingUtilities.convertPoint(e.getComponent(), e.getPoint(), _graphComponent.getGraphControl());
      ...
      _currentAngle = ((pt.getX() > cx) ? -1 : 1) * Math.acos(dy / c) + _PI4 + _initialAngle;

      dirty.add(Utils.getBoundingBox(_currentState, _currentAngle * Constants.DEG_PER_RAD));
      dirty.grow(1);
  ...
  void mouseReleased(MouseEvent e) {
    if (_graphComponent.isEnabled() && isEnabled() && !e.isConsumed() && _first != null) {
      ...
      if (_graphComponent.isEnabled() && isEnabled() && !e.isConsumed() && willExecute) {
        _graphComponent.getGraph().setCellStyles(Constants.STYLE_ROTATION, String.valueOf(deg), [cell]);
  ...
  void reset() {
    ...
    if (_currentState != null && _first != null) {
      dirty = Utils.getBoundingBox(_currentState, _currentAngle * Constants.DEG_PER_RAD);
      dirty.grow(1);
    }
  ...
  void paint(Graphics g) {
    if (_currentState != null && _first != null) {
      awt.Rectangle rect = _currentState.getRectangle();
      double deg = _currentAngle * Constants.DEG_PER_RAD;

      if (deg != 0) {
        (g as Graphics2D).rotate(Math.toRadians(deg), _currentState.getCenterX(), _currentState.getCenterY());
      }
