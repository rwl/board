...
class CompactTreeLayout extends GraphLayout {
  ...
  void execute(Object parent, [Object root = null]) {
    ...
    if (root == null) {
      ...
      if (graph.getEdges(parent, model.getParent(parent), _invert, !_invert, false).length > 0) {
        root = parent;
  ...
  _TreeNode _dfs(Object cell, Object parent, Set<Object> visited) {
    ...
    if (cell != null && !visited.contains(cell) && !isVertexIgnored(cell)) {
      ...
      IGraphModel model = graph.getModel();
      _TreeNode prev = null;
      List<Object> out = graph.getEdges(cell, parent, _invert, !_invert, false, true);
      ...
      for (int i = 0; i < out.length; i++) {
        ...
        if (!isEdgeIgnored(edge)) {
          ...
          Object target = (state != null) ? state.getVisibleTerminal(_invert) : view.getVisibleTerminal(edge, _invert);
  ...
  Rect _horizontalLayout(_TreeNode node, double x0, double y0, Rect bounds) {
    ...
    if (child != null) {
      ...
      while (s != null) {
        bounds = _horizontalLayout(s, node.x + child.offsetX, siblingOffset, bounds);
  ...
  Rect _verticalLayout(_TreeNode node, Object parent, double x0, double y0, Rect bounds) {
    ...
    if (child != null) {
      ...
      while (s != null) {
        bounds = _verticalLayout(s, node, siblingOffset, node.y + child.offsetX, bounds);
        siblingOffset += s.offsetY;
        s = s.next;
      }
    }
  ...
  Rect _apply(_TreeNode node, Rect bounds) {
    ...
    if (cell != null && g != null) {
      ...
      if (bounds == null) {
        bounds = new Rect(g.getX(), g.getY(), g.getWidth(), g.getHeight());
      } else {
        bounds = new Rect(Math.min(bounds.getX(), g.getX()), Math.min(bounds.getY(), g.getY()), Math.max(bounds.getX() + bounds.getWidth(), g.getX() + g.getWidth()), Math.max(bounds.getY() + bounds.getHeight(), g.getY() + g.getHeight()));
      }
    }
  ...
  void _processNodeOutgoing(_TreeNode node) {
    ...
    while (child != null) {
      ...
      sortedCells.add(new _WeightedCellSorter(child, sortingCriterion as int));
      child = child.next;
    }
