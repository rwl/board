...
class Curve
{
	...
	Line getCurveParallel(String index, double distance)
	{
				...
				&& curveIntervals != null && distance >= 0.0 && distance <= 1.0)
		{
			...
			double startPointY = firstPointOfSeg.getY() + segVectorY
					* distanceAlongSeg;
			Point2d endPoint = new Point2d(segVectorX / segLength, segVectorY
					/ segLength);
	...
	Point2d[] getCurveSection(String index, double start, double end)
	{
				...
				&& end >= 0.0 && end <= 1.0)
		{
			...
			double distanceAlongSeg = (start - curveIntervals[lowerLimit])
					/ (curveIntervals[lowerLimit + 1] - curveIntervals[lowerLimit]);
			Point2d startPoint = new Point2d(firstPointOfSeg.getX()
					...
					&& end > curveIntervals[lowerLimit - 1])
			{
				...
				Point2d endPoint = new Point2d(firstPointOfSeg.getX()
						+ segVectorX * distanceAlongSeg, firstPointOfSeg.getY()
						+ segVectorY * distanceAlongSeg);
				result.add(endPoint);
			}
	...
	bool intersectsRect(Rectangle rect)
	{
		...
		if (pointsCurve != null && pointsCurve.length > 1)
		{
			...
			{
				if (r.contains(pointsCurve[i].getX(),
						pointsCurve[i].getY())
						|| r.contains(pointsCurve[i - 1].getX(),
			...
			for (int i = 1; i < pointsCurve.length; i++)
			{
				if (r.intersectLine(pointsCurve[i].getX(),
	...
	Point2d collisionMove(String index, Rect rect, double buffer)
	{
		...
		else
		{
			...
			{
				hitPoint = Utils.intersection(x + width, y, x + width, y + height,
						x0, y0, x1, y1);
			}

			if (hitPoint == null)
			{
				hitPoint = Utils.intersection(x + width, y + height, x, y + height,
						x0, y0, x1, y1);
	...
	Rect getRelativeFromAbsPoint(Point2d absPoint, String index)
	{
		...
		return new Rect(distAlongEdge, orthogonalOffset, cartOffsetX,
				cartOffsetY);
	}
	...
	void _createCoreCurve()
	{
		...
		for (double t = 0; t <= 1.5; t += interval)
		{
			if (t > 1.0)
			{
				...
				t = 1.0001;
				Point2d endControlPoint = guidePoints
						.get(guidePoints.size() - 1);
				Point2d finalPoint = new Point2d(endControlPoint.getX(),
