/**
 * $Id: DefaultTextShape.java,v 1.1 2012/11/15 13:26:44 gaudenz Exp $
...
public class DefaultTextShape implements ITextShape
{
			...
			CellState state, Map<String, Object> style)
	{
		...
		if (g.getClipBounds() == null || g.getClipBounds().intersects(rect))
		{
			...
			int fontSize = Utils.getInt(style, Constants.STYLE_FONTSIZE,
			...
			y += 2 * fm.getMaxAscent() - fm.getHeight()
					+ Constants.LABEL_INSET * scale;

			Object vertAlign = Utils.getString(style,
					Constants.STYLE_VERTICAL_ALIGN, Constants.ALIGN_MIDDLE);
			...
			Object align = Utils.getString(style, Constants.STYLE_ALIGN,
